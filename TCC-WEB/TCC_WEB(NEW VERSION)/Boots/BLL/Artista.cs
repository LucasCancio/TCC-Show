...
namespace BLL
{
    public class Artista //OK
    {
        ...
        public DataSet ListarHistorico()
        {
            ...
            comando = "SELECT EV.URL_IMG, EV.ID_EVENTO, EV.TITULO, EV.DATA_EVENTO, CONCAT(CONCAT( FORMAT(EV.HORARIO_INICIO, 'HH:mm') ,' até '),FORMAT(EV.HORARIO_FINAL,'HH:mm')) as HORARIO, EV.DURACAO, EV.N_ARTISTAS,EV.DESCRICAO FROM TB_EVENTO EV INNER JOIN TB_EVENTO_ARTISTA EA ON EA.ID_EVENTO= EV.ID_EVENTO WHERE EA.ID_PESSOA= (SELECT P.ID_PESSOA FROM TB_PESSOA P INNER JOIN TB_ARTISTA_GERAL AG ON AG.ID_PESSOA= P.ID_PESSOA WHERE AG.ID_ARTISTA_GERAL= " + _ID_ARTISTA_GERAL+ ") AND EV.DATA_EVENTO >= CONVERT(DATETIME,'" + _DataListarInicio.ToString().Substring(0, 10) + "', 103) AND EV.DATA_EVENTO <= CONVERT(DATETIME,'" + DataListarFinal.ToString().Substring(0, 10) + "', 103)";
        ...
        public SqlDataReader ConsultarPorNome()
        {
            try
            {
                DAO.ClasseConexao c = new DAO.ClasseConexao();
                SQL = "SELECT AG.ID_ARTISTA_GERAL FROM TB_PESSOA P INNER JOIN TB_ARTISTA_GERAL AG ON AG.ID_PESSOA= P.ID_PESSOA WHERE P.NOME LIKE '%" + _Nome +"%'";
                return c.RetornarDataReader(SQL);
            }
            catch (Exception ex)
            {
        ...
        public SqlDataReader ConsultarFIXO()
        {
            try
            {
                DAO.ClasseConexao c = new DAO.ClasseConexao();
                ...
                case "Ambos":
                    if (tipodata != string.Empty && tipodata != null)//COM DATA ESPECIFICA
                    {
                        if (ativo == 2 && texto.Length == 0)
                        {
                            ...
                            comando = " SELECT (SELECT COUNT(ID_EVENTO) FROM TB_EVENTO_ARTISTA WHERE ID_PESSOA=P.ID_PESSOA) AS QTDE_EV,AG.ID_ARTISTA_GERAL,AG.URL_IMG, P.NOME, TP.TIPO_PESSOA, P.SEXO,  RD.FACEBOOK, RD.TWITTER, RD.INSTAGRAM, P.ATIVO, P.DATA_CRIACAO FROM TB_PESSOA P INNER JOIN TB_ARTISTA_GERAL AG ON AG.ID_PESSOA= P.ID_PESSOA INNER JOIN TB_REDESOCIAL RD ON RD.ID_REDESOCIAL= AG.ID_REDESOCIAL INNER JOIN TB_TIPO_PESSOA TP ON TP.ID_TIPO_PESSOA=P.ID_TIPO_PESSOA WHERE " + tipodata + ">= CONVERT(DATETIME,'" + _DataListarInicio.ToString().Substring(0, 10) + "', 103) AND " + tipodata + "<= CONVERT(DATETIME,'" + DataListarFinal.ToString().Substring(0, 10) + "', 103) ORDER BY AG.ID_ARTISTA_GERAL ";


                        }
                        else if (ativo == 2 && texto.Length != 0)
                        {
                            comando = "   SELECT (SELECT COUNT(ID_EVENTO) FROM TB_EVENTO_ARTISTA WHERE ID_PESSOA=P.ID_PESSOA) AS QTDE_EV,AG.ID_ARTISTA_GERAL,AG.URL_IMG, P.NOME, TP.TIPO_PESSOA, P.SEXO,  RD.FACEBOOK, RD.TWITTER, RD.INSTAGRAM, P.ATIVO, P.DATA_CRIACAO FROM TB_PESSOA P INNER JOIN TB_ARTISTA_GERAL AG ON AG.ID_PESSOA= P.ID_PESSOA INNER JOIN TB_REDESOCIAL RD ON RD.ID_REDESOCIAL= AG.ID_REDESOCIAL INNER JOIN TB_TIPO_PESSOA TP ON TP.ID_TIPO_PESSOA=P.ID_TIPO_PESSOA  WHERE " + tipo + " LIKE '%" + texto + "%' AND " + tipodata + ">= CONVERT(DATETIME,'" + _DataListarInicio.ToString().Substring(0, 10) + "', 103) AND " + tipodata + "<= CONVERT(DATETIME,'" + DataListarFinal.ToString().Substring(0, 10) + "', 103)   ORDER BY " + tipo + "";
                        ...
                        else
                        {
                            if (texto.Length == 0) // texto == null || texto == ""
                            {
                                ...
                                comando = "    SELECT (SELECT COUNT(ID_EVENTO) FROM TB_EVENTO_ARTISTA WHERE ID_PESSOA=P.ID_PESSOA) AS QTDE_EV,AG.ID_ARTISTA_GERAL,AG.URL_IMG, P.NOME, TP.TIPO_PESSOA, P.SEXO,   RD.FACEBOOK, RD.TWITTER, RD.INSTAGRAM, P.ATIVO, P.DATA_CRIACAO FROM TB_PESSOA P INNER JOIN TB_ARTISTA_GERAL AG ON AG.ID_PESSOA= P.ID_PESSOA INNER JOIN TB_REDESOCIAL RD ON RD.ID_REDESOCIAL= AG.ID_REDESOCIAL INNER JOIN TB_TIPO_PESSOA TP ON TP.ID_TIPO_PESSOA=P.ID_TIPO_PESSOA WHERE P.ATIVO=" + ativo + "  AND " + tipodata + ">= CONVERT(DATETIME,'" + _DataListarInicio.ToString().Substring(0, 10) + "', 103) AND " + tipodata + "<= CONVERT(DATETIME,'" + DataListarFinal.ToString().Substring(0, 10) + "', 103) ORDER BY " + tipo + "";

                            }
                            else
                            {
                                comando = "  SELECT (SELECT COUNT(ID_EVENTO) FROM TB_EVENTO_ARTISTA WHERE ID_PESSOA=P.ID_PESSOA) AS QTDE_EV,AG.ID_ARTISTA_GERAL,AG.URL_IMG, P.NOME, TP.TIPO_PESSOA, P.SEXO,  RD.FACEBOOK, RD.TWITTER, RD.INSTAGRAM, P.ATIVO, P.DATA_CRIACAO FROM TB_PESSOA P INNER JOIN TB_ARTISTA_GERAL AG ON AG.ID_PESSOA= P.ID_PESSOA INNER JOIN TB_REDESOCIAL RD ON RD.ID_REDESOCIAL= AG.ID_REDESOCIAL INNER JOIN TB_TIPO_PESSOA TP ON TP.ID_TIPO_PESSOA=P.ID_TIPO_PESSOA WHERE " + tipo + " LIKE '%" + texto + "%' AND " + tipodata + ">= CONVERT(DATETIME,'" + _DataListarInicio.ToString().Substring(0, 10) + "', 103) AND " + tipodata + "<= CONVERT(DATETIME,'" + DataListarFinal.ToString().Substring(0, 10) + "', 103) AND P.ATIVO = " + ativo + " ORDER BY " + tipo + "";
                ...
                case "Artista":
                    if (tipodata != string.Empty && tipodata != null)//COM DATA ESPECIFICA
                    {
                        if (ativo == 2 && texto.Length == 0)
                        {
                            ...
                            comando = " SELECT (SELECT COUNT(ID_EVENTO) FROM TB_EVENTO_ARTISTA WHERE ID_PESSOA=P.ID_PESSOA) AS QTDE_EV,AG.ID_ARTISTA_GERAL,AG.URL_IMG, P.NOME, TP.TIPO_PESSOA, P.SEXO, AGT.NOME_PRINCIPAL, AGT.DOCUMENTO, RD.FACEBOOK,RD.TWITTER, RD.INSTAGRAM, P.ATIVO, P.DATA_CRIACAO FROM TB_PESSOA P INNER JOIN TB_ARTISTA_GERAL AG ON AG.ID_PESSOA= P.ID_PESSOA INNER JOIN TB_REDESOCIAL RD ON RD.ID_REDESOCIAL= AG.ID_REDESOCIAL INNER JOIN TB_TIPO_PESSOA TP ON TP.ID_TIPO_PESSOA=P.ID_TIPO_PESSOA INNER JOIN TB_ARTISTA ART ON ART.ID_ARTISTA_GERAL= AG.ID_ARTISTA_GERAL INNER JOIN TB_AGENTE AGT ON AGT.ID_AGENTE= ART.ID_AGENTE WHERE " + tipodata + ">= CONVERT(DATETIME,'" + _DataListarInicio.ToString().Substring(0, 10) + "', 103) AND " + tipodata + "<= CONVERT(DATETIME,'" + DataListarFinal.ToString().Substring(0, 10) + "', 103) ORDER BY AG.ID_ARTISTA_GERAL ";


                        }
                        else if (ativo == 2 && texto.Length != 0)
                        {
                            comando = " SELECT (SELECT COUNT(ID_EVENTO) FROM TB_EVENTO_ARTISTA WHERE ID_PESSOA=P.ID_PESSOA) AS QTDE_EV,AG.ID_ARTISTA_GERAL,AG.URL_IMG, P.NOME, TP.TIPO_PESSOA, P.SEXO, AGT.NOME_PRINCIPAL, AGT.DOCUMENTO, RD.FACEBOOK,RD.TWITTER, RD.INSTAGRAM, P.ATIVO, P.DATA_CRIACAO FROM TB_PESSOA P INNER JOIN TB_ARTISTA_GERAL AG ON AG.ID_PESSOA= P.ID_PESSOA INNER JOIN TB_REDESOCIAL RD ON RD.ID_REDESOCIAL= AG.ID_REDESOCIAL INNER JOIN TB_TIPO_PESSOA TP ON TP.ID_TIPO_PESSOA=P.ID_TIPO_PESSOA INNER JOIN TB_ARTISTA ART ON ART.ID_ARTISTA_GERAL= AG.ID_ARTISTA_GERAL INNER JOIN TB_AGENTE AGT ON AGT.ID_AGENTE= ART.ID_AGENTE  WHERE " + tipo + " LIKE '%" + texto + "%'  AND " + tipodata + ">= CONVERT(DATETIME,'" + _DataListarInicio.ToString().Substring(0, 10) + "', 103) AND " + tipodata + "<= CONVERT(DATETIME,'" + DataListarFinal.ToString().Substring(0, 10) + "', 103)  ORDER BY " + tipo + "";
                        ...
                        else
                        {
                            if (texto.Length == 0) // texto == null || texto == ""
                            {
                                ...
                                comando = "   SELECT (SELECT COUNT(ID_EVENTO) FROM TB_EVENTO_ARTISTA WHERE ID_PESSOA=P.ID_PESSOA) AS QTDE_EV,AG.ID_ARTISTA_GERAL,AG.URL_IMG, P.NOME, TP.TIPO_PESSOA, P.SEXO, AGT.NOME_PRINCIPAL, AGT.DOCUMENTO, RD.FACEBOOK,RD.TWITTER, RD.INSTAGRAM, P.ATIVO, P.DATA_CRIACAO FROM TB_PESSOA P INNER JOIN TB_ARTISTA_GERAL AG ON AG.ID_PESSOA= P.ID_PESSOA INNER JOIN TB_REDESOCIAL RD ON RD.ID_REDESOCIAL= AG.ID_REDESOCIAL INNER JOIN TB_TIPO_PESSOA TP ON TP.ID_TIPO_PESSOA=P.ID_TIPO_PESSOA INNER JOIN TB_ARTISTA ART ON ART.ID_ARTISTA_GERAL= AG.ID_ARTISTA_GERAL INNER JOIN TB_AGENTE AGT ON AGT.ID_AGENTE= ART.ID_AGENTE WHERE P.ATIVO=" + ativo + " AND " + tipodata + ">= CONVERT(DATETIME,'" + _DataListarInicio.ToString().Substring(0, 10) + "', 103) AND " + tipodata + "<= CONVERT(DATETIME,'" + DataListarFinal.ToString().Substring(0, 10) + "', 103) ORDER BY " + tipo + "";
                ...
                case "ArtistaFixo":
                    if (tipodata != string.Empty && tipodata != null)//COM DATA ESPECIFICA
                    {
                        if (ativo == 2 && texto.Length == 0)
                        {
                            ...
                            comando = " SELECT (SELECT COUNT(ID_EVENTO) FROM TB_EVENTO_ARTISTA WHERE ID_PESSOA=P.ID_PESSOA) AS QTDE_EV,AG.ID_ARTISTA_GERAL,AG.URL_IMG, P.NOME, TP.TIPO_PESSOA, P.SEXO, AF.CPF, RD.FACEBOOK,RD.TWITTER, RD.INSTAGRAM, P.ATIVO, P.DATA_CRIACAO FROM TB_PESSOA P INNER JOIN TB_ARTISTA_GERAL AG ON AG.ID_PESSOA= P.ID_PESSOA INNER JOIN TB_REDESOCIAL RD ON RD.ID_REDESOCIAL= AG.ID_REDESOCIAL INNER JOIN TB_TIPO_PESSOA TP ON TP.ID_TIPO_PESSOA=P.ID_TIPO_PESSOA INNER JOIN TB_ARTISTA_FIXO AF ON AF.ID_ARTISTA_GERAL= AG.ID_ARTISTA_GERAL WHERE " + tipodata + ">= CONVERT(DATETIME,'" + _DataListarInicio.ToString().Substring(0, 10) + "', 103) AND " + tipodata + "<= CONVERT(DATETIME,'" + DataListarFinal.ToString().Substring(0, 10) + "', 103) ORDER BY AG.ID_ARTISTA_GERAL ";


                        }
                        else if (ativo == 2 && texto.Length != 0)
                        {
                            comando = " SELECT (SELECT COUNT(ID_EVENTO) FROM TB_EVENTO_ARTISTA WHERE ID_PESSOA=P.ID_PESSOA) AS QTDE_EV,AG.ID_ARTISTA_GERAL,AG.URL_IMG, P.NOME, TP.TIPO_PESSOA, P.SEXO, AF.CPF, RD.FACEBOOK,RD.TWITTER, RD.INSTAGRAM, P.ATIVO, P.DATA_CRIACAO FROM TB_PESSOA P INNER JOIN TB_ARTISTA_GERAL AG ON AG.ID_PESSOA= P.ID_PESSOA INNER JOIN TB_REDESOCIAL RD ON RD.ID_REDESOCIAL= AG.ID_REDESOCIAL INNER JOIN TB_TIPO_PESSOA TP ON TP.ID_TIPO_PESSOA=P.ID_TIPO_PESSOA INNER JOIN TB_ARTISTA_FIXO AF ON AF.ID_ARTISTA_GERAL= AG.ID_ARTISTA_GERAL  WHERE " + tipo + " LIKE '%" + texto + "%' AND " + tipodata + ">= CONVERT(DATETIME,'" + _DataListarInicio.ToString().Substring(0, 10) + "', 103) AND " + tipodata + "<= CONVERT(DATETIME,'" + DataListarFinal.ToString().Substring(0, 10) + "', 103)  ORDER BY " + tipo + "";
                        ...
                        else
                        {
                            if (texto.Length == 0) // texto == null || texto == ""
                            {
                                ...
                                comando = "   SELECT (SELECT COUNT(ID_EVENTO) FROM TB_EVENTO_ARTISTA WHERE ID_PESSOA=P.ID_PESSOA) AS QTDE_EV,AG.ID_ARTISTA_GERAL,AG.URL_IMG, P.NOME, TP.TIPO_PESSOA, P.SEXO, AF.CPF, RD.FACEBOOK,RD.TWITTER, RD.INSTAGRAM, P.ATIVO, P.DATA_CRIACAO FROM TB_PESSOA P INNER JOIN TB_ARTISTA_GERAL AG ON AG.ID_PESSOA= P.ID_PESSOA INNER JOIN TB_REDESOCIAL RD ON RD.ID_REDESOCIAL= AG.ID_REDESOCIAL INNER JOIN TB_TIPO_PESSOA TP ON TP.ID_TIPO_PESSOA=P.ID_TIPO_PESSOA INNER JOIN TB_ARTISTA_FIXO AF ON AF.ID_ARTISTA_GERAL= AG.ID_ARTISTA_GERAL WHERE P.ATIVO=" + ativo + " AND " + tipodata + ">= CONVERT(DATETIME,'" + _DataListarInicio.ToString().Substring(0, 10) + "', 103) AND " + tipodata + "<= CONVERT(DATETIME,'" + DataListarFinal.ToString().Substring(0, 10) + "', 103) ORDER BY " + tipo + "";
        ...
        public SqlDataReader VerificarEvento()
        {
            try
            {
                DAO.ClasseConexao c = new DAO.ClasseConexao();
        ...
        public void Artista_crud(string opr)
        {
            try
            {
                ...
                p.Add(new SqlParameter("@ID_REDESOCIAL", SqlDbType.Int) { Value = _ID_REDESOCIAL });
                p.Add(new SqlParameter("@FACEBOOK", SqlDbType.VarChar) { Value = _Facebook });
                p.Add(new SqlParameter("@TWITTER", SqlDbType.VarChar) { Value = _Twitter });
                p.Add(new SqlParameter("@INSTAGRAM", SqlDbType.VarChar) { Value = _INSTAGRAM });
                ...
                p.Add(new SqlParameter("@URL_IMG", SqlDbType.VarChar) { Value = _URL_IMG });
                p.Add(new SqlParameter("@ID_AGENTE", SqlDbType.Int) { Value = _ID_AGENTE });
                p.Add(new SqlParameter("@ID_ENDERECO", SqlDbType.Int) { Value = _ID_ENDERECO });
                p.Add(new SqlParameter("@TELEFONE", SqlDbType.VarChar) { Value = _Telefone });
