    ...
    public class Evento
    {
        ...
        public DataSet ListarGeral(string texto, int ativo, string tipo, int fID, int fNArt, string fTipoPag)
        {
            ...
            else if (ativo == 2 && texto.Length != 0)
            {
                if (Where != string.Empty)
                {
                    comando = " SELECT EV.ID_EVENTO,EV.TITULO, EV.DATA_EVENTO, SUBSTRING(CAST(EV.HORARIO_INICIO AS VARCHAR),10,5), SUBSTRING(CAST(EV.HORARIO_FINAL AS VARCHAR),10,5),EV.DURACAO, EV.N_ARTISTAS,EV.DESCRICAO,EV.TIPO_PAG, EV.ATIVO, EV.VALOR_EVENTO FROM TB_EVENTO EV " + Where + " AND " + tipo + " LIKE '%" + texto + "%' AND EV.DATA_EVENTO >= CONVERT(DATETIME,'" + _DataListarInicio.ToString().Substring(0, 10) + "', 103) AND EV.DATA_EVENTO <= CONVERT(DATETIME,'" + DataListarFinal.ToString().Substring(0, 10) + "', 103) ORDER BY " + tipo + "";
                }
                else
                {
            ...
            else
            {
                ...
                else
                {
                    if (Where != string.Empty)
                    {
                        comando = " SELECT EV.ID_EVENTO,EV.TITULO, EV.DATA_EVENTO, SUBSTRING(CAST(EV.HORARIO_INICIO AS VARCHAR),10,5), SUBSTRING(CAST(EV.HORARIO_FINAL AS VARCHAR),10,5),EV.DURACAO, EV.N_ARTISTAS,EV.DESCRICAO,EV.TIPO_PAG, EV.ATIVO, EV.VALOR_EVENTO FROM TB_EVENTO EV  " + Where + " AND " + tipo + " LIKE '%" + texto + "%' AND EV.ATIVO = " + ativo + " AND EV.DATA_EVENTO >= CONVERT(DATETIME,'" + _DataListarInicio.ToString().Substring(0, 10) + "', 103) AND EV.DATA_EVENTO <= CONVERT(DATETIME,'" + DataListarFinal.ToString().Substring(0, 10) + "', 103) ORDER BY " + tipo + "";
                    }
                    else
                    {
